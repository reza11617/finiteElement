...
void StiffnessMatrixFirstOrder::DOFCreator(unsigned int numberElement, unsigned int noIP, unsigned int nip,  unsigned int stiffMatSizePerElement, unsigned int *mesh, unsigned int *i_index, unsigned int *j_index)
{
  // size of DOF is like the size of stiffness Matrix
  unsigned int counter = stiffMatSizePerElement*(numberElement*nip+noIP);
  unsigned int xi, xj, yi, yj;
  for (unsigned int i = 0; i<8; i++)
    for (unsigned int j = 0; j<i+1; j++)
      {
	xi = i/2; yi = i%2-1; xj = j/2; yj = j%2-1;
	i_index[counter]   = (mesh[numberElement*4+xi]+1)*2+yi;
	j_index[counter++] = (mesh[numberElement*4+xj]+1)*2+yj;
      }  
};
