  ...
  for (unsigned int i = 0; i<simulationPerThread[threadId]; i++)
    {
      ...
      DOFCreator(numberElement,noIP,nipSquared, sizeStiffMatPerEle, geometry->mesh, stiffMat->i, stiffMat->j);
      stiffnessMatrixCalculation(numberElement, noIP,  nipSquared,integrationNode, integrationPos, integrationWeight, c, material->materialMatrix, stiffMat->value);
    }
