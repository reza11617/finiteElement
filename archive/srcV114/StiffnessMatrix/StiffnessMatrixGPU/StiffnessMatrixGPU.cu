...
__global__ void StiffnessMatrixKernel(unsigned int n, unsigned int nip, float* in, unsigned int* ip, float* iw, float* c, float* D, unsigned int* mesh, float* k, unsigned int* i_index, unsigned int *j_index, unsigned int* rowPtr, unsigned int* dofFree,StiffnessMatrixGPU *obj)
{
  ...
  for (int i = index; i < n; i += stride)
    {
      obj->stiffnessMatrixCalculation(i, nip, in, ip, iw, c, D, mesh, k, i_index, j_index, rowPtr, dofFree);
    }
}
...
Sparse& StiffnessMatrixGPU::GetStiffnessMatrix()
{
  ...
  StiffnessMatrixKernel<<<numBlocks, blockSize>>>(numberOfElements, nipSquared, integrationNode, integrationPos, integrationWeight, c, D_d, geometry->get_mesh(), stiffMat->value, stiffMat->i, stiffMat->j , stiffMat->nnz_inRow, geometry->get_Dof().get_free(),this);
