...
unsigned int Element::concurentThreadsSupported = (std::thread::hardware_concurrency()-1);
  ...
  : material(&mat), geometry(&geo), numberOfIntegrationPoint(n)
{
  ...
  stiffMat = new double[stiffMatSize];
  c = new double[numberOfElements*6];
  //fuctions
  integrationPoint();
  // multi thread processeign
  threadInitializer();
  MultiCoreSimulation();
  SingleCoreSimulation();
  

  //Log::Logger().Info(stiffMat,36);
};
...
void Element::MultiCoreSimulation()
{
  Log::Logger().Info("time for MultiCoreSimulation function");
  Timer timer;
  ...
  for (unsigned int i = 0; i<concurentThreadsSupported; i++)
    {
      t[i] = std::thread(&Element::stiffnessMatrixFirstOrderForEachThread,this,i);
    }
  for (unsigned int i = 0; i<concurentThreadsSupported; i++)
    {
      t[i].join();
...
void Element::threadInitializer()
{
...
void Element::stiffnessMatrixFirstOrderForEachThread(unsigned int threadId)
{
...
{
  integrationNode   = new double[numberOfIntegrationPoint];
  integrationPos    = new unsigned int[numberOfIntegrationPoint*dimention*numberOfIntegrationPoint];
  integrationWeight = new double[numberOfIntegrationPoint];
