...
__global__ void StiffnessMatrixKernel(unsigned int n, unsigned int nip, float* in, unsigned int* ip, float* iw, float* c, float* D, float* k, unsigned int sizePerEle, unsigned int* mesh , unsigned int * i_index, unsigned int *j_index,StiffnessMatrixGPU *obj)
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
  for (int i = index; i < n; i += stride)
    {
      ...
      obj->DOFCreator(numberElement, noIP, nip, sizePerEle, mesh, i_index,j_index);
...
Sparse& StiffnessMatrixGPU::GetStiffnessMatrix()
{
  ...
  */
  ...
  StiffnessMatrixKernel<<<numBlocks, blockSize>>>(simulationSize, nipSquared, integrationNode_d, integrationPos_d, integrationWeight_d, c_d, D_d, stiffMat_d->value, sizeStiffMatPerEle, mesh_d, stiffMat_d->i,stiffMat_d->j ,this);
  cudaDeviceSynchronize();
  return *stiffMat_d;
}
