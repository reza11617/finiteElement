...
int main() {
  ...
  *p_cantilever = geometry();
  ...
  //delete element;
  //Log::Logger().Info(element1->stiffMat, 4);
  Element* element1 = new ElementCuda(mat,*p_cantilever,4);
  element1->GpuSimulations();
  StiffnessMatrix* stiffMat = new StiffnessMatrixFirstOrder(mat,*p_cantilever,4);
  //Log::Logger().Info(stiffMat->GetStiffnessMatrix(), 4);
  StiffnessMatrixFirstOrder* stiffMatParCPU = new StiffnessMatrixParallelCPU(mat,*p_cantilever,4,5);
  //Log::Logger().Info(stiffMatParCPU->GetStiffnessMatrix(), 4);
  StiffnessMatrixFirstOrder* stiffMatGPU = new StiffnessMatrixGPU(mat,*p_cantilever,4);
  //Log::Logger().Info(stiffMatGPU->GetStiffnessMatrix(), 4);
  std::cout << maxError(*stiffMatGPU,*stiffMat) << " is the Max Error"<<std::endl;
  delete stiffMat;
...
float maxError(StiffnessMatrix& A, StiffnessMatrix& B)
{
  float* ka = A.GetStiffnessMatrix();
  float* kb = B.GetStiffnessMatrix();
  float MaxError = 0.00;
  for (int i = 0; i < A.GetStiffnessMatrixSize(); i++)
    MaxError = fmax(MaxError, fabs(ka[i] - kb[i]));
  return MaxError;
}
