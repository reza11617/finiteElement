...
Sparse& StiffnessMatrixFirstOrder::GetStiffnessMatrix()
{
...
void StiffnessMatrixFirstOrder::DOFCreator(unsigned int numberElement, unsigned int noIP)
{
  ...
  for (unsigned int i = 0; i<8; i++)
	...
	stiffMat->i[counter]   = (geometry->mesh[numberElement*4+xi]+1)*2+yi;
	stiffMat->j[counter++] = (geometry->mesh[numberElement*4+xj]+1)*2+yj;
      }  
};
