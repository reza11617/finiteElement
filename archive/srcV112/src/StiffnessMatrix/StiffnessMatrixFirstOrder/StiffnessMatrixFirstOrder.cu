...
void StiffnessMatrixFirstOrder::stiffnessMatrixCalculation(unsigned int numberElement, unsigned int nip ,float* in, unsigned int* ip, float* iw, float* c, float* D, unsigned int* mesh, float* k, unsigned int* i_index, unsigned int* j_index, unsigned int* rowPtr)
...
// rowptr -> pointer to an array holding the size of stiffness matrix in each row
{
  ...
  for (unsigned int noIP = 0; noIP < nip; noIP++) { 
    ...
    kLocal[2]  = kLocal[2]  + WeightPerDetJ*(D[0]*N[1] + D[4]*N[5] + D[4]*N[11] + D[2]*N[27]);//3,1
    kLocal[3]  = kLocal[3]  + WeightPerDetJ*(D[4]*N[1] + D[3]*N[5] + D[2]*N[11] + D[5]*N[27]);//4,1
    kLocal[4]  = kLocal[4]  + WeightPerDetJ*(D[0]*N[2] + D[4]*N[6] + D[4]*N[17] + D[2]*N[28]);//5,1
    kLocal[5]  = kLocal[5]  + WeightPerDetJ*(D[4]*N[2] + D[3]*N[6] + D[2]*N[17] + D[5]*N[28]);//6,1
    kLocal[6]  = kLocal[6]  + WeightPerDetJ*(D[0]*N[3] + D[4]*N[7] + D[4]*N[22] + D[2]*N[29]); //7,1
    kLocal[7]  = kLocal[7]  + WeightPerDetJ*(D[4]*N[3] + D[3]*N[7] + D[2]*N[22] + D[5]*N[29]);//8,1
    kLocal[8]  = kLocal[8]  + WeightPerDetJ*(D[2]*N[0] + 2*D[5]*N[4] + D[1]*N[26]);           //2,2
    kLocal[9]  = kLocal[9]  + WeightPerDetJ*(D[4]*N[1] + D[3]*N[11] + D[2]*N[5] + D[5]*N[27]);//3,2
    kLocal[10] = kLocal[10] + WeightPerDetJ*(D[2]*N[1] + D[5]*N[5] + D[5]*N[11] + D[1]*N[27]);//4,2
    kLocal[11] = kLocal[11] + WeightPerDetJ*(D[4]*N[2] + D[3]*N[17] + D[2]*N[6] + D[5]*N[28]);//5,2
    kLocal[12] = kLocal[12] + WeightPerDetJ*(D[2]*N[2] + D[5]*N[6] + D[5]*N[17] + D[1]*N[28]);//6,2
    kLocal[13] = kLocal[13] + WeightPerDetJ*(D[4]*N[3] + D[3]*N[22] + D[2]*N[7] + D[5]*N[29]); //7,2
    kLocal[14] = kLocal[14] + WeightPerDetJ*(D[2]*N[3] + D[5]*N[7] + D[5]*N[22] + D[1]*N[29]);//8,2
    kLocal[15] = kLocal[15] + WeightPerDetJ*(D[0]*N[8] + 2*D[4]*N[12] + D[2]*N[30]);          //3,3
    kLocal[16] = kLocal[16] + WeightPerDetJ*(D[4]*N[8] + D[5]*N[30] + D[3]*N[12] + D[2]*N[12]);//4,3
    kLocal[17] = kLocal[17] + WeightPerDetJ*(D[0]*N[9] + D[4]*N[13] + D[4]*N[18] + D[2]*N[31]);//5,3
    kLocal[18] = kLocal[18] + WeightPerDetJ*(D[4]*N[9] + D[3]*N[13] + D[2]*N[18] + D[5]*N[31]);//6,3
    kLocal[19] = kLocal[19] + WeightPerDetJ*(D[0]*N[10] + D[4]*N[14] + D[4]*N[23] + D[2]*N[32]);//7,3
    kLocal[20] = kLocal[20] + WeightPerDetJ*(D[4]*N[10] + D[3]*N[14] + D[2]*N[23] + D[5]*N[32]);//8,3
    kLocal[21] = kLocal[21] + WeightPerDetJ*(D[2]*N[8] + 2*D[5]*N[12] + D[1]*N[30]);           //4,4
    kLocal[22] = kLocal[22] + WeightPerDetJ*(D[4]*N[9] + D[3]*N[18] + D[2]*N[13] + D[5]*N[31]);//5,4
    kLocal[23] = kLocal[23] + WeightPerDetJ*(D[2]*N[9] + D[5]*N[13] + D[5]*N[18] + D[1]*N[31]);//6,4
    kLocal[24] = kLocal[24] + WeightPerDetJ*(D[4]*N[10] + D[3]*N[23] + D[2]*N[14] + D[5]*N[32]);//7,4
    kLocal[25] = kLocal[25] + WeightPerDetJ*(D[2]*N[10] + D[5]*N[14] + D[5]*N[23] + D[1]*N[32]);//8,4
    kLocal[26] = kLocal[26] + WeightPerDetJ*(D[0]*N[15] + 2*D[4]*N[19] + D[2]*N[33]);//5,5
    kLocal[27] = kLocal[27] + WeightPerDetJ*(D[4]*N[15] + D[5]*N[33] + D[3]*N[19] + D[2]*N[19]);//6,5
    kLocal[28] = kLocal[28] + WeightPerDetJ*(D[0]*N[16] + D[4]*N[20] + D[4]*N[24] + D[2]*N[34]);//7,5
    kLocal[29] = kLocal[29] + WeightPerDetJ*(D[4]*N[16] + D[3]*N[20] + D[2]*N[24] + D[5]*N[34]);//8,5
    kLocal[30] = kLocal[30] + WeightPerDetJ*(D[2]*N[15] + 2*D[5]*N[19] + D[1]*N[33]);//6,6
    kLocal[31] = kLocal[31] + WeightPerDetJ*(D[4]*N[16] + D[3]*N[24] + D[2]*N[20] + D[5]*N[34]);//7,6
    kLocal[32] = kLocal[32] + WeightPerDetJ*(D[2]*N[16] + D[5]*N[20] + D[5]*N[24] + D[1]*N[34]);//8,6
    kLocal[33] = kLocal[33] + WeightPerDetJ*(D[0]*N[21] + 2*D[4]*N[25] + D[2]*N[35]);//7,7
  ...
  for (unsigned int i = 0; i<8; i++) {
    for (unsigned int j = i; j<8; j++) {
      dof_i = (mesh[numberElement*4+i/2])*2+i%2+1;
      dof_j = (mesh[numberElement*4+j/2])*2+j%2+1;
      i_index[counter] = min(dof_i,dof_j);
      j_index[counter] = max(dof_i,dof_j);
