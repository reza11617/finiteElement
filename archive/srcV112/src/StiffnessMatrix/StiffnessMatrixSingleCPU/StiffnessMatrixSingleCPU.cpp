...
Sparse& StiffnessMatrixSingleCPU::GetStiffnessMatrix()
{
  ...
  for (unsigned int i = 0; i<numberOfElements; i++)
    stiffnessMatrixCalculation(i, nipSquared,integrationNode, integrationPos, integrationWeight, c, material->materialMatrix, geometry->get_mesh(), stiffMat->value, stiffMat->i, stiffMat->j, stiffMat->rowPtr);
  return *stiffMat;
}
