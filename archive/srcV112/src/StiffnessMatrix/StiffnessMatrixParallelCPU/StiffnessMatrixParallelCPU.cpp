...
void StiffnessMatrixParallelCPU::GetStiffnessMatrixForEachThread(unsigned int threadId)
{
  ...
  for (unsigned int i = 0; i<simulationPerThread[threadId]; i++)
    stiffnessMatrixCalculation(counter+i, nipSquared,integrationNode, integrationPos, integrationWeight, c, material->materialMatrix, geometry->get_mesh(), stiffMat->value, stiffMat->i, stiffMat->j, stiffMat->rowPtr);
}
