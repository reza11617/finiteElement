...
unsigned int* Geometry::get_nodeReAppear() { return nodeReAppear;}
// Struct Laod
Load::Load() {
}
Load::~Load() {
...
void Load::build(unsigned int nOfNodes) {
  ...
  loadSparse_d = new Sparse(LoadVector_dof_i.size(), 2*nOfNodes, 1);

  cudaMemcpy(loadSparse_d->value, loadVec, nOfNodes*sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(loadSparse_d->i, loadDof, nOfNodes*sizeof(unsigned int), cudaMemcpyHostToDevice);
